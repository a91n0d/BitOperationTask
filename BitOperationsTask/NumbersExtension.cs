...
namespace BitOperationsTask
{
    ...
    public static class NumbersExtension
    {
        ...
        public static int InsertNumberIntoAnother(int destinationNumber, int sourceNumber, int i, int j)
        {
            if (i < 0 || i > 31)
            {
                throw new ArgumentOutOfRangeException(nameof(i), "i is less than 0 or more than 31.");
            }

            if (j < 0 || j > 31)
            {
                throw new ArgumentOutOfRangeException(nameof(j), "j is less than 0 or more than 31.");
            }
